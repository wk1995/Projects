...
import com.wk.projects.common.time.date.DateTime.getDateString
import com.wk.projects.common.time.date.DateTime.getTime
...
class SchedulesMainAdapter(val itemList: ArrayList<ScheduleItem>)
    ...
    override fun convert(helper: BaseViewHolder?, item: ScheduleItem?) {
        item?.run {
            ...
            //表示只是时刻，不是具体的项目
            if (itemName.isEmpty()) {
                helper?.setText(R.id.tvScheduleItemName, getDateString(startTime, defaultSimpleDateFormat))
            ...
            helper?.setTextColor(R.id.tvScheduleItemName,
                    if (finish) {
                        Color.BLACK
                    } else {
                        Color.RED
                    ...
                    ?.setText(R.id.tvScheduleItemTime,
                            if (finish) {
                                ...
                                getTime(System.currentTimeMillis() - (item.startTime))
                            }
                    )
